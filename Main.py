...
async def on_ready():
    ...
    async def Ticket():
        ...
        async def button_callback(interaction):
            ...
            overwrites = {
                ...
                interaction.user: discord.PermissionOverwrite(
                    read_messages=True
                ),  # Allow the command author to read messages
            }
        ...
        ticketembed.set_thumbnail(
            url="https://www.citypng.com/public/uploads/preview/-11597269407aqavkzrcos.png"
        )
    ...
    async def get_dm():
        ...
        button = discord.ui.Button(
            label="Recieve D.M.", style=discord.ButtonStyle.secondary
        )
    ...
    async def create_team():
        ...
        button1 = discord.ui.Button(
            label="Create Team", style=discord.ButtonStyle.success
        )
        ...
        button2 = discord.ui.Button(
            label="Join Team", style=discord.ButtonStyle.blurple
        )

        async def button2_callback(interaction):
            await interaction.response.defer()
            print("join")
            message4 = await interaction.followup.send(
                f"you have joined a team", ephemeral=True
            )
            ...
            for member in ctx.channel.members:
                ...
                if role in member.roles:
                    ...
                    await ctx.channel.edit(
                        name=f"{old_name}-closed",
                        overwrites=overwrites,
                        category=discord.utils.get(
                            guild.categories, id=1214909385600671745
                        ),
                    )
        else:
...
@bot.command()
async def hello(ctx):
    greetings = discord.Embed(
        title=f"Hello from {bot.user.display_name}", color=0x00F069
    )
    try:
...
async def kill(ctx):
    ...
    killembed.set_thumbnail(
        url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTJGvnN7ou7F3pkGQQpIFtYNvbrmb5eiXGFPw&s"
    )
    button2 = discord.ui.Button(
        label="!!! SHUTDOWN !!!", style=discord.ButtonStyle.danger
    )
    ...
    async def button2_callback(interaction):
        ...
        message4 = await interaction.followup.send(
            f"The bot is shutting down", ephemeral=True
        )
        await bot.close()
    ...
    if not isinstance(ctx.channel, discord.DMChannel):
        ...
        if required_role in ctx.author.roles:
            ...
            print(
                f"{ctx.author.name} tried to shut down the bot without permission! -> id : {ctx.author.id}"
            )
            return
...
bot.run("MTIwNjYzMDk4MzYzNDI1NTg3Mg.GsWQWT.IJJOG9_itxhLkx8sJQfy_UsVGb1DNEZDiH9_9Y")
