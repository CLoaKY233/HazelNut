...
def send_invite(reg_no, teamname, inviter):
    workbook = openpyxl.load_workbook('data.xlsx')
    sheet = workbook.active
  ...
  <body>
    <div class="container">
      ...
      <p>Hello {name}, someone is expecting you to join their team! </p>
      <p>To join the team, please copy the INVITER's name and TEAMCODE given below!</p>
      <h2 style="text-align: center; background-color: #E0E0E0; padding: 10px; border-radius: 5px;">Inviter: {inviter}</h2>

      <h3 style="text-align: center; background-color: #E0E0E0; padding: 10px; border-radius: 5px;">Team-code: {invite_code}</h3>
  ...
  """
    ...
    # Create message container - the correct MIME type is multipart/alternative.
    msg = MIMEMultipart('alternative')
    msg['Subject'] = 'You have received a new invite!'
    msg['From'] = "exhibitionease.auth@gmail.com"
    msg['To'] = email
